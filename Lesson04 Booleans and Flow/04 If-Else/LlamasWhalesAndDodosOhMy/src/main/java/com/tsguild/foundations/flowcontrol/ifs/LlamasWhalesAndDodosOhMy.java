...
public class LlamasWhalesAndDodosOhMy {
    public static void main(String[] args) {
        ...
        if (dodos < 0) {
            System.out.println("Hold on, how do we have NEGATIVE dodos?");
        }
        if (llamas > whales) {
            System.out.println("Whales may be bigger, but llamas are better... ha!");
        }
        if (llamas <= whales) {
            System.out.println("Brawn over brains, eh?  Whales beat llamas");
        }
        
        System.out.println("There's been a huge increase in the dodo population due to cloning!");
        dodos += 100;
