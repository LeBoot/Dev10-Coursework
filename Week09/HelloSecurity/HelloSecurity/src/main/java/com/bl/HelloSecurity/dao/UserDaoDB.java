...
import com.bl.HelloSecurity.dao.RoleDaoDB.RoleMapper;
...
public class UserDaoDB implements UserDao {
    ...
    private Set<RoleObject> getRolesForUser(int id) throws DataAccessException {
        final String SELECT_ROLES_FOR_USER = "SELECT r.* FROM user_role ur "
                + "JOIN role r ON ur.role_id = r.id "
                + "WHERE ur.user_id = ?";
        Set<RoleObject> roles = new HashSet(jdbc.query(SELECT_ROLES_FOR_USER, new RoleMapper(), id));
        return roles;
    }
