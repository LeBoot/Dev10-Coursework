...
public class MainController {
    ...
    public String viewInventory(Integer id, Model model) {
        Store store = (Store) stores.findById(id).orElse(null);
    ...
    public String addProduct(Product product, HttpServletRequest request) {
        ...
        Store store = (Store) stores.findById(storeId).orElse(null);
    ...
    public String displayProduct(Integer id, Model model) {
        Product product = (Product) products.findById(id).orElse(null);
    ...
    public String addSupplier(Supplier supplier, HttpServletRequest request) {
        ...
        Product product = (Product) products.findById(productId).orElse(null);
        
        supplier = (Supplier) suppliers.save(supplier);
    ...
    public String addExistingSupplier(Integer productId, Integer supplierId) {
        Product product = (Product) products.findById(productId).orElse(null);
        Supplier supplier = (Supplier) suppliers.findById(supplierId).orElse(null);
    ...
    public String displaySupplier(Integer id, Model model) {
        Supplier supplier = (Supplier) suppliers.findById(id).orElse(null);
